...
- (UIView *)createStarViewWithImage:(NSString *)imageName {
    ...
    for (NSInteger i = 0; i < self.numberOfStars; i ++)
    {
        ...
        imageView.frame = CGRectMake(i * 14, 0, self.bounds.size.width / self.numberOfStars, self.bounds.size.height);
...
- (void)layoutSubviews {
    ...
    [UIView animateWithDuration:animationTimeInterval animations:^{
       weakSelf.foregroundStarView.frame = CGRectMake(0, 0, weakSelf.bounds.size.width * weakSelf.scorePercent - 1, weakSelf.bounds.size.height);
    }];
}
