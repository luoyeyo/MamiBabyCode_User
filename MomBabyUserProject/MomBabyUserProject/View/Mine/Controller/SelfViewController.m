...
- (void)createUI {
    ...
    if (kUserInfo.isLogined) {
        infoArr = @[kUserInfo.nickname, ageStr, kUserInfo.residence];
        if (kUserInfo.status == kUserStateChild) {
            infoArr = @[babyName, kUserInfo.nickname, ageStr, kUserInfo.residence];
            // 有宝宝
            titleArr = @[@"头像", @"宝宝姓名", @"昵称", @"年龄", @"居住地址"];
            placeholderArr = @[@"头像", @"请输入宝宝姓名", @"输入昵称", @"年龄", @"选择居住地址"];
            _haveBaby = YES;
        }
    } else {
        infoArr = @[@"", @"", @""];
    ...
    for (int i = 0; i < titleArr.count; i ++) {
        ...
        else
        {
            ...
            InputLimitField *textField = [[InputLimitField alloc] initWithFrame:CGRectMake(textFieldX, textFieldY, textFieldW, textFieldH)];
...
- (void)updateUserInfo
{
    ...
    UITextField *nickNameTF = (UITextField *)[self.view viewWithTag:1 + index];
    UITextField *ageTF = (UITextField *)[self.view viewWithTag:2 + index];
    UITextField *residenceTF = (UITextField *)[self.view viewWithTag:3 + index];
    ...
    UITextField *babyNameTF;
    if (_haveBaby) {
        babyNameTF = (UITextField *)[self.view viewWithTag:1];
