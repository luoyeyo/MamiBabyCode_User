...

{
    ...
    FMResultSet *result =  [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ LIMIT 150",tableName]];
...
-(NSArray *)DataBase:(FMDatabase *)db selectKeyTypes:(NSDictionary *)keyTypes fromTable:(NSString *)tableName whereConditionRange:(NSDictionary *)condition;
...
- (BOOL)deleteDatabase {
    ...
    NSError *error = nil;
    [fileManager removeItemAtPath:[kBaseSQL databasePath] error:&error];
    if (error) {
        NSLog(@"error: ______    %@",error.localizedDescription);
        return NO;
    }
    return YES;
}
